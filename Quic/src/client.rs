use crate::{
    ...
    UpstreamType, GAMING_STREAM_WINDOW_SIZE, GAMING_RECEIVE_WINDOW_SIZE, GAMING_SEND_WINDOW_SIZE, GAMING_MAX_CONCURRENT_STREAMS,
};
...
impl Client {
    ...
    async fn prepare_login_config(&self) -> Result<LoginConfig> {
        let mut transport_cfg = self.create_transport_config();
    ...
    async fn serve_outbound_tcp(
        ...
        let tcp_server = inner_state!(self, tcp_servers)
            .get(&local_server_addr)
            .cloned()
            .context("tcp server not found")?;

        TcpTunnel::start(
            true,
            &conn,
            &mut tcp_server.clone(),
            pending_tcp_stream,
            self.config.tcp_timeout_ms,
            self.config.gaming_mode,
        )
        .await;
    ...
    async fn serve_outbound_udp(
        ...
        let udp_server = inner_state!(self, udp_servers)
            .get(&local_server_addr)
            .cloned()
            .context("udp server not found")?;

        UdpTunnel::start(&conn, udp_server, None, self.config.udp_timeout_ms, self.config.gaming_mode)
            .await?;
    ...
    async fn serve_inbound_tcp(
        ...
        TcpTunnel::process(&conn, local_server_addr, self.config.tcp_timeout_ms, self.config.gaming_mode).await;
        Ok(())
    }
    ...
    async fn serve_inbound_udp(
        ...
        UdpTunnel::process(&conn, local_server_addr, self.config.udp_timeout_ms, self.config.gaming_mode).await;
        Ok(())
    }
    ...
    fn create_client_config_builder(
        ...
        rustls::Error,
    > {
        let crypto_provider = self.get_crypto_provider(cipher);
    ...
    fn create_transport_config(&self) -> TransportConfig {
        ...
        // Use gaming-optimized settings if gaming mode is enabled
        if self.config.gaming_mode {
            transport_cfg.stream_receive_window(quinn::VarInt::from_u32(GAMING_STREAM_WINDOW_SIZE));
            transport_cfg.receive_window(quinn::VarInt::from_u32(GAMING_RECEIVE_WINDOW_SIZE));
            transport_cfg.send_window(GAMING_SEND_WINDOW_SIZE);
            transport_cfg.max_concurrent_bidi_streams(VarInt::from_u32(GAMING_MAX_CONCURRENT_STREAMS));
        } else {
