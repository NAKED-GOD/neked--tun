...
// Gaming-optimized constants for lower latency and memory usage
pub const GAMING_UDP_PACKET_SIZE: usize = 1024; // Reduced from 1500 for gaming
pub const GAMING_TCP_BUFFER_SIZE: usize = 4096; // Reduced from 8192 for gaming
pub const GAMING_QUIC_TIMEOUT_MS: u64 = 15000; // Reduced from 30000 for faster reconnection
pub const GAMING_TCP_TIMEOUT_MS: u64 = 10000; // Reduced from 30000 for faster timeout
pub const GAMING_UDP_TIMEOUT_MS: u64 = 2000; // Reduced from 5000 for faster timeout
pub const GAMING_STREAM_WINDOW_SIZE: u32 = 512 * 1024; // Reduced from 1024*1024 for lower memory
pub const GAMING_RECEIVE_WINDOW_SIZE: u32 = 1024 * 1024; // Reduced from 2*1024*1024
pub const GAMING_SEND_WINDOW_SIZE: u32 = 1024 * 1024; // Reduced from 2*1024*1024
pub const GAMING_MAX_CONCURRENT_STREAMS: u32 = 256; // Reduced from 1024 for gaming

lazy_static! {
    static ref BUFFER_POOL: BytePool::<Vec<u8>> = BytePool::<Vec<u8>>::new();
    // Gaming-optimized buffer pool with smaller buffers
    static ref GAMING_BUFFER_POOL: BytePool::<Vec<u8>> = BytePool::<Vec<u8>>::new();
}
...
impl ClientConfig {
    pub fn create(
        ...
        if quic_timeout_ms == 0 {
            quic_timeout_ms = if gaming_mode { GAMING_QUIC_TIMEOUT_MS } else { 30000 };
        }
        if tcp_timeout_ms == 0 {
            tcp_timeout_ms = if gaming_mode { GAMING_TCP_TIMEOUT_MS } else { 30000 };
        }
        if udp_timeout_ms == 0 {
            udp_timeout_ms = if gaming_mode { GAMING_UDP_TIMEOUT_MS } else { 5000 };
        }

        let mut tunnels = Vec::new();
        if !tcp_addr_mappings.is_empty() {
            parse_addr_mappings(tcp_addr_mappings, UpstreamType::Tcp, &mut tunnels)?;
        }
        if !udp_addr_mappings.is_empty() {
            parse_addr_mappings(udp_addr_mappings, UpstreamType::Udp, &mut tunnels)?;
        }
...
pub mod android {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_initCertificateVerifier(
        mut env: JNIEnv,
        _: JClass,
        context: JObject,
    ) {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeInitLogger(
        mut env: JNIEnv,
        _: JClass,
        jlogLevel: JString,
    ) -> jboolean {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeCreate(
        mut env: JNIEnv,
        _: JClass,
        jserverAddr: JString,
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeStop(
        _env: JNIEnv,
        _: JClass,
        client_ptr: jlong,
    ) {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeStartTunnelling(
        _env: JNIEnv,
        _: JClass,
        client_ptr: jlong,
    ) {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeGetState(
        env: JNIEnv,
        _: JClass,
        client_ptr: jlong,
    ) -> jstring {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn Java_com_rstun_RsTunc_nativeSetEnableOnInfoReport(
        env: JNIEnv,
        jobj: JClass,
        client_ptr: jlong,
        enable: jboolean,
